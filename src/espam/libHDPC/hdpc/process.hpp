/* $Id: process.hpp,v 1.1 2009/10/21 10:30:35 nikolov Exp $ */
/* $license$ */
#pragma once
...
namespace hdpc {
	...
	template <class PLATFORM, int INPORT_COUNT, int OUTPORT_COUNT> class Process {
	public:
		typedef enum readError {READ_FINISHED, READ_ERROR};
		...
		HANDLE start(DWORD_PTR cpu_mask, PLATFORM &a, EXEC& process, bool allocmem = false);
		inline void inc_execution_cntr();
		void waitforfinish();

		template <lock::lock_t lock_t, typename token_t> inline bool readFromPort(size_t port, token_t &element) throw(...);
		template <lock::lock_t lock_t, typename token_t> inline bool writeToPort(size_t port, const token_t &element) throw(...);

		template <lock::lock_t lock_t, typename token_t> inline const token_t &getReadPointer(size_t port) throw(...);
		template <lock::lock_t lock_t, typename token_t> inline token_t &getWritePointer(size_t port) throw(...);
		template <lock::lock_t lock_t, typename token_t> inline void releaseReadPointer(size_t port) throw(...);
		template <lock::lock_t lock_t, typename token_t> inline void releaseWritePointer(size_t port) throw(...);

		void releasePorts() throw(...);
	...
	private:
		template <lock::lock_t lock_t, typename token_t> inline Channel<token_t, lock_t>* getPort(ChannelBase *port) throw(...);

		bool allocMemToProcess() throw(...);
		bool freeMemFromProcess() throw(...);
		bool checkConnections();
		void finish(std::stringstream& buf);

		static DWORD WINAPI executeIntermediate(LPVOID param) throw(...);
	...
	template <class PLATFORM, int INPORT_COUNT, int OUTPORT_COUNT>
	HANDLE Process<PLATFORM, INPORT_COUNT, OUTPORT_COUNT>::start(DWORD_PTR cpu_mask, PLATFORM &a, EXEC& process, bool allocmem = false) {
		if (!checkConnections()) return false;
		callback = &a;
		do_work  = &process;

		use_device_memory = allocmem;
		h = CreateThread(NULL, 0, Process::executeIntermediate, this, 0, NULL);
		if (cpu_mask != 0x0) SetThreadAffinityMask(h, cpu_mask);
		return h;
	}

	template <class PLATFORM, int INPORT_COUNT, int OUTPORT_COUNT>
	void Process<PLATFORM, INPORT_COUNT, OUTPORT_COUNT>::releasePorts() {
		/* XXX - hack, lost type information, but these should be type-agonistic */
		for (int i = 0; i != inPorts.size(); i++) {
			if (HDPC_DEBUG_MODE && in[i] > 1 && inPorts[i]->lock_type != channel::lock::SPIN_ACQUIRE) {
				debug::error("%s must be used, possible deadlock",  inPorts[i]->lock());
				throw Process::READ_ERROR;
			}
			while (in[i] != 0) releaseReadPointer<void*>(i);
		}

		for (int i = 0; i != outPorts.size(); i++) {
			if (HDPC_DEBUG_MODE && out[i] > 1 && inPorts[i]->lock_type != channel::lock::SPIN_ACQUIRE) {
				debug::error("%s must be used, possible deadlock",  outPorts[i]->lock());
				throw Process::READ_ERROR;
			}
			while (out[i] != 0) releaseWritePointer<void*>(i);
		}
	}
	...
	template <class PLATFORM, int INPORT_COUNT, int OUTPORT_COUNT>
	DWORD WINAPI Process<PLATFORM, INPORT_COUNT, OUTPORT_COUNT>::executeIntermediate(LPVOID param) {
		hdpc::Process<PLATFORM, INPORT_COUNT, OUTPORT_COUNT> *p = static_cast<hdpc::Process<PLATFORM, INPORT_COUNT, OUTPORT_COUNT>*>(param);
