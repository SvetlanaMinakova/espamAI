...
void Convolution28::main(void *threadarg) {
  ...
  // while (1) {
    ...
    for (int rep = 0; rep < q ; rep ++) {
      ...
      if(IP0_tokens>0)
    	  readSWF_CPU(Convolution28_IP0_buf_ptr->fifo, &input[0][0] + IP0_shift*sizeof(int), IP0_tokens, Convolution28_IP0_buf_ptr->fifo_size);

   //   cout<<"read Conv28 "<<rep<<endl;
      //execution
      //execution parameters
        for (int n = 0; n < 8; n++) {
        appFunc::execute(std::string("CONV(5_5)"));
      }
