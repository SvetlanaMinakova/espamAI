...
public class CPPDNNVisitorARMCL extends CNNGraphVisitor{
    ...
    * Pads should contain values >=0
    ...
    private void _processPads(CNNNeuron x) {
        ...
        if(!(parent.isNullorEmptyPads())) {
            ...
            _printStream.print((pads[0] + pads[2]) / 2 + ", " + (pads[1] + pads[3])/2);
            return;
        }
    ...
    public void visitComponent(Arithmetic x) {
        ...
        // *Const mode TODO: expand to non-const mode
        //if (inputsNum == 1){
                _printStream.println(_prefix + "<< BatchNormalizationLayer(get_weights_accessor(data_path, \" \"),");
                _prefixInc();
                _prefixInc();
