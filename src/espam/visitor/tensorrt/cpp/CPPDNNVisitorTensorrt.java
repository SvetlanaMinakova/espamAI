...
public class CPPDNNVisitorTensorrt extends CNNGraphVisitor {
    ...
    public void callDNNVisitor(Network dnn, String dir) {
                callDNNVisitor(dnn, dir, false);
    }

    /**
     * Call CPP SDFG Visitor of the operational node (node, performs some useful job)
     * @param GPUProfile profile GPU and write results to console
     * @param dnn DNN
     * @param dir directory for .cpp templates
     */
    public void callDNNVisitor(Network dnn, String dir, boolean GPUProfile) {
        String stage = "beginning";
        try {
            ...
            //writeMain(y);
           // System.out.println("partition: "+ dnn.getName() + " generated ");
        } catch (Exception e) {
            System.err.println(".cpp file creation error for DNN/parition " + dnn.getName() + " stage = " + stage + e.getMessage());
        }
    }
...
///////////////////        ONE       ENGINE    //////////////////////////////////
   ...
   // std::cout<<"network 1 destroyed!"<<std::endl;
    ...
    public void visitComponent(Reshape x) {
        ...
        if (inputsNum == 1){
            String singleInput = inputNames.firstElement();
            String inputRef = singleInput.replace("->getOutput(0)", "");
            _printStream.println(_prefix + "auto " + layerName + " = " + inputRef + ";");
            return;
        }
        else System.err.println("Tensorrt reshape layer generation error: layer has " + inputsNum
                + " inputs, while 1 input expected.");
    }
    ...
    public void visitComponent(Arithmetic x) {
        ...
        // *Const mode
        if (inputsNum == 1){
            _printStream.println(_prefix + "auto " + layerName + " = network->addScale(*" + inputNames.firstElement() + ", ScaleMode::kUNIFORM," +
    ...
    public void visitComponent(Concat x) {
        ...
        for(int i=0; i<inputNames.size()-1; i++) {
            ...
            //System.out.println("DEFINED BINDING: " + inputNames.elementAt(i));
        }

        _printStream.println(inputNames.lastElement() + "}};");
        //System.out.println("DEFINED BINDING: " + inputNames.lastElement());
    ...
    public void simulateDataLayer(Neuron x, DataType dt) {
        Layer layer = x.getParent();
        switch (dt){
            case INPUT:{
                _simulateInputDataLayer(layer);
                break;
            }

            case OUTPUT:{
                _simulateOutputDataLayer(layer);
                break;
            }
    ...
    private void _simulateInputDataLayer(Layer l){
        String inputLayerName = _getInputLayerName(l);
        String inputLayerNameDef = inputLayerName.replace("->getOutput(0)", "");

            _printStream.println(_prefix + "// Create input tensor(s)");
            if (l.getNeuron() instanceof MultipleInputsProcessor) {
                Vector<Layer> inputLayers = ((MultipleInputsProcessor) l.getNeuron()).getInputOwners();
                Vector<String> inputLayerNames = _getInputLayerNamesList(l);
                String lname;

                int lId = 0;
                for (Layer input : inputLayers){
                        lname = inputLayerNames.elementAt(lId);
                        lname = lname.replace("->getOutput(0)", "");
                        _printStream.println(_prefix + "ITensor* " + lname + " = network->addInput(\"" + lname + "\", dt, Dims4{1, " + input.getOutputChannels() + ", " + input.getOutpH() + ", " + input.getOutpW() + "});");
                        _printStream.println(_prefix + "assert(" + lname + ");");
                        _simulatedDataLayers.add(lname);
                        lId++;
                }
            }
            else {
    ...
    private void _simulateOutputDataLayer(Layer l){
    ...
    * Pads should contain values >=0
    ...
    private void _processPads(CNNNeuron x) {
        ...
        if(!(parent.isNullorEmptyPads())) {
            ...
            int pads_x = (pads[1] + pads[3])/2;
            int pads_y = (pads[0] + pads[2])/2;
            _printStream.println(_prefix + layerName + "->setPadding(DimsHW{" + pads_x + ", " + pads_y + "});");
        }
    ...
    protected String _getInputLayerName(Layer layer){
        ...
        if(layer.getInputConnections().size()>0){
            ...
            if(!_isSimulatedDataLayer(inputLayer) && !(src.getNeuron() instanceof Data))
    ...
    protected Vector<String> _getInputLayerNamesList(Layer layer){
        ...
        if(namesList.size()==0 && layer.getNeuron() instanceof MultipleInputsProcessor){

            for (int i = 0; i < ((MultipleInputsProcessor) layer.getNeuron()).getInputOwners().size(); i++) {
                Layer src = ((MultipleInputsProcessor) layer.getNeuron()).getInputOwners().elementAt(i);
                inputLayer = src.getName();
                if (src.getNeuron().getNonlin() != null)
                    inputLayer += src.getNeuron().getNonlin();
    ...
    /** check if layer is a simulated data layer*/
    private boolean _isSimulatedDataLayer(String layerName){
        for (String sdl: _simulatedDataLayers){
            if (layerName.equals(sdl))
                return true;
        }
...
///////////////////        STANDARD LINES   //////////////////////////////////
     ...
     * @param dnn DNN
     */
    protected void _initParams(Network dnn, boolean GPUProfile) {
    ...
    if(GPUProfile)
        _printStream.println(_prefix +" this->detailed_profile = true;");

    _printStream.println(_prefix +" this->IObindings = " +_getIOBindingsNum(inputLayer) + ";");

    String inputBlobName = _getInputBlobName(inputLayer);
    _printStream.println(_prefix +" this->INPUT_BLOB_NAME = \"" + inputBlobName + "\";");

    _defineAdditionalInputBlobs(inputLayer);

    String outputBlobName = _getOutputBlobName(outputLayer);
    _printStream.println(_prefix +" this->OUTPUT_BLOB_NAME = \"" + outputBlobName + "\";");

    Integer INPUT_C = _getPartitionInputChannels(inputLayer);
    _printStream.println(_prefix + " this->INPUT_C = " + INPUT_C + ";");
    ...
    private void _defineAdditionalInputBlobs(Layer inputLayer){
        if (!(inputLayer.getNeuron() instanceof MultipleInputsProcessor))
            return;

        Vector<String> additionalBlobNames = _getAdditionalInputBlobNames(inputLayer);
        for (String blobName : additionalBlobNames)
            _printStream.println(_prefix +" this->ADDITIONAL_INPUT_BLOB_NAMES.push_back(\"" + blobName + "\");");

        Vector<Layer> inpLayers = ((MultipleInputsProcessor) inputLayer.getNeuron()).getInputOwners();

        int h, w, c;
        for (int i = 1; i<inpLayers.size(); i++){
            Layer additionalInputLayer = inpLayers.elementAt(i);
            h = additionalInputLayer.getOutpH();
            w = additionalInputLayer.getOutpW();
            c = additionalInputLayer.getOutputChannels();

            _printStream.println(_prefix +" this->ADDITIONAL_INPUT_H.push_back("+ h +");");
            _printStream.println(_prefix +" this->ADDITIONAL_INPUT_W.push_back("+ w +");");
            _printStream.println(_prefix +" this->ADDITIONAL_INPUT_C.push_back("+ c +");");
     ...
     * @return number of I/O s of a given layer
     */
    private Integer _getIOBindingsNum(Layer layer){
        if (!(layer.getNeuron() instanceof MultipleInputsProcessor))
            return 2;

        Integer inpLayersNum = ((MultipleInputsProcessor) layer.getNeuron()).getInputOwners().size();
        return inpLayersNum + 1;
    }

    /**
     * Get number of input channels for partition input layer
     * @ param inputLayer partition input layer
     * @return number of input channels for partition input layer
     */
    private Integer _getPartitionInputChannels(Layer inputLayer){
        /** TODO: refactoring*/

        if (inputLayer.getNeuron() instanceof MultipleInputsProcessor){
            Vector<Layer> inpLayers = ((MultipleInputsProcessor) inputLayer.getNeuron()).getInputOwners();
            if (inpLayers == null) return inputLayer.getInputChannels();
            if (inpLayers.size() == 0) return inputLayer.getInputChannels();
            Layer firstLayer = inpLayers.elementAt(0);
            return firstLayer.getOutputChannels();
        }

        if (inputLayer.getNeuron() instanceof DenseBlock){
            inputLayer.initOperatorIfNULL();
            int inputChannels = inputLayer.getInputChannels();
            return  inputChannels;
        }

        return inputLayer.getInputChannels();
    }




    /** GET name of the partition input blob
     * @param inputLayer partition input layer
     * @return name of the partition input blob
     */
    private String _getInputBlobName(Layer inputLayer){
        String defaultName = inputLayer.getName() + "_input";

        if(inputLayer.getNeuron() instanceof Data)
            return inputLayer.getName();

        if (inputLayer.getNeuron() instanceof MultipleInputsProcessor){
            Vector<Layer> inpLayers = ((MultipleInputsProcessor) inputLayer.getNeuron()).getInputOwners();
            if (inpLayers==null) return defaultName;
            if (inpLayers.size() == 0) return defaultName;
            Layer fisrtLayer = inpLayers.firstElement();
            String inputLayerName = fisrtLayer.getName();
            if(fisrtLayer.getNeuron().getNonlin()!=null)
                inputLayerName += fisrtLayer.getNeuron().getNonlin();
            //System.out.println("BINDING TO FIND : " + inputLayerName);
     ...
     * @return name of the partition input blob
     */
    private Vector<String> _getAdditionalInputBlobNames(Layer inputLayer){
        Vector<String> additionalInputBlobNames = new Vector<>();
        if (!(inputLayer.getNeuron() instanceof MultipleInputsProcessor))
            return additionalInputBlobNames;


            Vector<Layer> inpLayers = ((MultipleInputsProcessor) inputLayer.getNeuron()).getInputOwners();
            if (inpLayers==null) return additionalInputBlobNames;

            for (int i = 1; i<inpLayers.size(); i++){

                Layer additionalInputLayer = inpLayers.elementAt(i);
                String inputLayerName = additionalInputLayer.getName();
                if(additionalInputLayer.getNeuron().getNonlin()!=null)
                    inputLayerName += additionalInputLayer.getNeuron().getNonlin();

                additionalInputBlobNames.add(inputLayerName);
            }
    ...
    /** GET name of the partition output blob
     * @param outputLayer partition output layer
     * @return name of the partition output blob
     */
    private String _getOutputBlobName(Layer outputLayer){
        if(outputLayer.getNeuron() instanceof Data)
            return outputLayer.getName();
        else return outputLayer.getName() + "_output";
    }
     ...
     * @param dnn DNN
     */
    protected void _generateDummyWeights(Network dnn) throws Exception {
        ...
        for(Layer l: dnn.getLayers()) {
            ...
            try {
                ...
                if (l.getNeuron() instanceof Convolution || l.getNeuron() instanceof DenseBlock) {
                    ...
                    l.getNeuron().updateWeights(l.getInputChannels(), l.getOutputChannels());


                    //if(tensorParams.containsKey("weights"))
                ...
                System.err.println("Layer " + l.getName() + "(op = " +l.getNeuron().getName() + ")" + " weights update error.");
            }
        }
        _printStream.println();
    ...
    /** added external input layers*/
    private Vector<String> _simulatedDataLayers = new Vector<>();
