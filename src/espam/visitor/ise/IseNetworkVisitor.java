 ...
 * @version $Id: IseNetworkVisitor.java,v 1.15 2012/02/24 13:27:55 svhaastr Exp $
 */
...
public class IseNetworkVisitor extends PlatformVisitor {
  ...
  public void visitComponent(Platform x) {
    try {
      ...
      _stopConjuctionS = "  ---- Conjunction of all stop signals\n" +
                         "  done <= '1'";
      _externalFifoDecls = "";
      ...
      _systemPS.println("    done        : out std_logic");
      _systemPS.println("  );");
      ...
      _systemPS.println(_stopConjuctionS + ";\n");
      ...
      if (IseConfig.genSimul() == true) {
        ...
        _iseScriptPS.println("");
        PrintStream simscriptPS = _openFile("runsim.tcl");
        PrintStream simcmdPS = _openFile("simtb.cmd");
        _writeSimulationScripts(simscriptPS, simcmdPS);
      }
  ...
  public void visitComponent(CompaanHWNode x) {
    ...
    if (!IseConfig.omitIONodes() || !_isSource(_adgNode) && !_isSink(_adgNode)) {
      ...
      _siglistS += "  signal s_" + _adgNode.getName() + "_STOP: std_logic;\n";
      _stopConjuctionS += " and s_" + _adgNode.getName() + "_STOP";
    }
  }
...
//        }
  ...
  /**
   * Writes a simulation script and a simulation command file.
   */
  private void _writeSimulationScripts(PrintStream simscriptPS, PrintStream simcmdPS) {
    // Write simulation command file
    simcmdPS.println("onerror {resume}");
    simcmdPS.println("run all;");
    simcmdPS.println("exit;");

    // Write simulation script
    simscriptPS.println("# Automatically generated by the ESPAM ISE visitor");
    simscriptPS.println("# This Tcl script should be executed using the Xilinx Tcl Shell (xtclsh) to run the simulation.");
    simscriptPS.println("");
    simscriptPS.println("project open " + _projectName);
    simscriptPS.println("project set top \"simtb\"");
    simscriptPS.println("");
    simscriptPS.println("# We need a .prj file, which will be generated when we run a syntax check.");
    simscriptPS.println("process run \"Behavioral Check Syntax\" -instance simtb");
    simscriptPS.println("");
    simscriptPS.println("# Fixup the .prj file a bit to make it suitable for FUSE.");
    simscriptPS.println("exec sed -i \"s/isim_temp/work/\" simtb_stx_beh.prj");
    simscriptPS.println("");
    simscriptPS.println("# Run fuse");
    simscriptPS.println("set fusestatus [catch {exec fuse -prj simtb_stx_beh.prj -o tb.exe -top simtb} fuseresult]");
    simscriptPS.println("");
    ...
    simscriptPS.println("project close");
  }
  ...
  private void _writeSimulationTB(PrintStream simtbPS) {
    ...
    simtbPS.println("      done        : out std_logic");
    simtbPS.println("    );");
  ...
  private void _writeComponentInst() {
    ...
    _modinstS += "      STOP => s_" + _adgNode.getName() + "_STOP\n";
    _modinstS += "    );\n";
    _modinstS += "\n";
  }
  ...
  private String _stopConjuctionS;
