...
public class CrossbarVisitor extends PlatformVisitor {
	...
	hdlPS.println("  SIGNAL sl_req_valid   : STD_LOGIC_VECTOR(ports_num-1 DOWNTO 0);");
	...
	hdlPS.println("           END IF;");
	hdlPS.println("");
	hdlPS.println("           IF( sl_req_regs(I)(31) = '0' ) THEN");
	hdlPS.println("              sl_clear_flag(I) <= '1';");
	hdlPS.println("           END IF;");
	hdlPS.println("");
	hdlPS.println("           IF(  sl_req_valid(I) = '0' ) THEN");
	...
	hdlPS.println("     ELSIF rising_edge(CLK) THEN");
	hdlPS.println("");
	hdlPS.println("        IF( sl_req_regs(sl_cur_req)(31) = '1' AND");
	hdlPS.println("            sl_clear_flag(sl_cur_req) = '0'   AND");
	hdlPS.println("            req_accepted(sl_cur_req) = false  AND");
	hdlPS.println("            sl_cross_table( CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1 ) = '0' ) THEN");
	hdlPS.println("");
	hdlPS.println("            sl_req_valid(sl_cur_req) <= '1';");
	hdlPS.println("");
	hdlPS.println("            -- accept the request and check 'empty' signal");
	...
	hdlPS.println("");
	hdlPS.println("        ELSIF( sl_clear_flag(sl_cur_req) = '1'  AND");
	hdlPS.println("               sl_req_valid(sl_cur_req) = '1' ) THEN  -- clear connection");
	hdlPS.println("");
	hdlPS.println("            sl_cross_table( CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1) <= '0'; -- the conn. resource is free");
	hdlPS.println("            C_o(sl_cur_req) <= (OTHERS => '0');");
	hdlPS.println("            C_i(CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1) <= (OTHERS => '0');");
	hdlPS.println("");
	...
	hdlPS.println("            END IF;");
	hdlPS.println("");
	hdlPS.println("        ELSIF( req_accepted(sl_cur_req) = TRUE ) THEN  -- check the correspondent 'empty'");
	hdlPS.println("");
	hdlPS.println("            if( F_empty_i( CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1 ) = '0' ) THEN -- make connection");
	hdlPS.println("                C_o(sl_cur_req) <= sl_req_regs(sl_cur_req)(ports_num_log2+15 DOWNTO 16);	-- src");
	hdlPS.println("                C_i(CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1) <= CONV_STD_LOGIC_VECTOR(sl_cur_req+1, ports_num_log2); --dest");
	hdlPS.println("");
	hdlPS.println("                sl_cross_table(CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1) <= '1';");
	hdlPS.println("                sl_req_valid(sl_cur_req) <= '1';");
	hdlPS.println("            ELSE");
	hdlPS.println("                C_o(sl_cur_req) <= (OTHERS => '0');");
	hdlPS.println("                C_i(CONV_INTEGER(sl_req_regs(sl_cur_req)(30 DOWNTO 16))-1) <= (OTHERS => '0');");
