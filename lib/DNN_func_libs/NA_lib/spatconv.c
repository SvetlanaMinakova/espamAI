...
printf("inside spatconv_sw\n");
printf("IF %d, OF %d, height %d, width %d, kern height %d, kern width %d\n",in_s[0], out_s[0], out_s[1], out_s[2], kern_s[0], kern_s[1]);
      ...
      for (ITER wout = 0; wout < out_s[2]; wout++) {
        ...
        for (ITER pin = 0; pin < IF; pin++) {
          ...
          for (ITER hkern = 0; hkern < kern_s[0]; hkern++) {
            ...
            int hin = stride[0] * hout + hkern - pad[0];
            ...
            for (ITER wkern = 0; wkern < kern_s[1]; wkern++) {
              ...
              // calculate required input position 
              int win = stride[1] * wout + wkern - pad[1];
          ...
          if(kern_s[0]==3 || kern_s[0]==5 || kern_s[0]==1) {
            ...
            if(((kern_s[0] == 3 || kern_s[0]==1) && if_count == _MIN_IF) || (kern_s[0] == 5 && if_count == _MIN_IF)) {
              if((saturate(mac >> qf, "Conv")) == _MAX_ || (saturate(mac >> qf, "Conv")) == _MIN_)
                 sat_count++;
...
RET spatconv_forward_sw(CONVLAYER sc, DATA* kernel, DATA* output, DATA* input, SIZE in_s[3],
    ...
    _tprintf_("[%dx%d] software spatial convolution:\n", kern_s[2], kern_s[2]);
    _tprintf_("\t%lu to %lu\n", in_s[0], out_s[0]);
    _tprintf_("\t%lux%lu to %lux%lu\n", in_s[1], in_s[2], out_s[1], out_s[2]);
 
  printf("spatconv\n");
	_tcreate_(time);
	spatconv_sw(input, output, kernel, sc->bias, in_s, out_s, &kern_s[2], pad, stride, activate, ncol, qf);
...
CONVLAYER convlayer_create(void) {
    CONVLAYER sc = (CONVLAYER) calloc(1, sizeof(struct Convlayer));
    return sc;
}
