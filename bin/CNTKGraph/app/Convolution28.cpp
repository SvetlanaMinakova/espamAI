...
Convolution28::Convolution28() : csdfNode() {
  //assign FIFO sizes
  IP0_fifo_size = 3920;
  OP1_fifo_size = 6272;
...
//const int parameters
  ...
  int_params["input_dim_3"] = input_dim_3;
...
Convolution28::~Convolution28() {}

void Convolution28::main(void *threadarg) {
  // create communication channel
  thread_info *thread_data;
  thread_data = (struct thread_info *) threadarg;
  fifo_buf* Convolution28_IP0_buf_ptr = thread_data->get_fifo_buf_by_dst("Convolution28_IP0");
 
  fifo_buf* Convolution28_OP1_buf_ptr = thread_data->get_fifo_buf_by_src("Convolution28_OP1");
  ...
  // while (1) {
    ...
    for (int rep = 0; rep < q ; rep ++) {
      ...
      if ( IP0_tokens > 0 )
        readSWF_CPU(Convolution28_IP0_buf_ptr->fifo, &input[0][0][0][0] + IP0_shift*sizeof(int), IP0_tokens, Convolution28_IP0_buf_ptr->fifo_size);

      //execution
      //execution parameters
        for (int n = 0; n < 8; n++) {
        appFunc::execute(std::string("CONV(5_5)"),&input[0][0][0][0], &weights[n][0][0], &output[n][0][0], &int_params);
      }
      ...
      if ( OP1_tokens > 0 )
        writeSWF_CPU(Convolution28_OP1_buf_ptr->fifo, &output[0][0][0], OP1_tokens, Convolution28_OP1_buf_ptr->fifo_size);
    }// loop over the phases
  cout<<" Convolution28 finished! "<<endl;
  //} while (1)
} // main
